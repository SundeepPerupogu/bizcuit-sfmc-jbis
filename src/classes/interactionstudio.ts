...
export default class InteractionStudio {
    ...
    public async executeApi(payload: any): Promise<InteractionStudioResponse> {
        ...
        if (this.config.token !== '') {
            ...
            request.headers = { ...request.headers, Authorization: `Basic ${buffer.toString('base64')}` };
        }
        ...
        try {
            ...
            isResponse.status = 'ERROR: IS API call failed with message: ' + (err as Error).message;
